Profile::Admin.controllers :posts do
  get :index do
    @title = "Publicaciónes"
    @posts = Post.all
    render 'posts/index'
  ...
  post :create do
    ...
    if @post.save
      ...
      flash[:success] = pat(:create_success, :model => 'Publicación')
  ...
  put :update, :with => :id do
    ...
    if @post
      if @post.update_attributes(params[:post])
        flash[:success] = pat(:update_success, :model => 'Publicación', :id =>  "#{params[:id]}")
  ...
  delete :destroy, :with => :id do
    ...
    if post
      if post.destroy
        flash[:success] = pat(:delete_success, :model => 'Publicación', :id => "#{params[:id]}")
      else
  ...
  delete :destroy_many do
    ...
    if Post.destroy posts
      ...
      flash[:success] = pat(:destroy_many_success, :model => 'Publicación', :ids => "#{ids.join(', ')}")
    end
    redirect url(:posts, :index)
  end
end
