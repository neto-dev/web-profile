Profile::Admin.controllers :accounts do
  get :index do
    @title = "Cuentas"
  ...
  post :create do
    ...
    if @account.save
      ...
      flash[:success] = pat(:create_success, :model => 'Cuenta')
  ...
  put :update, :with => :id do
    ...
    if @account
      if @account.update_attributes(params[:account])
        flash[:success] = pat(:update_success, :model => 'Cuenta', :id =>  "#{params[:id]}")
  ...
  delete :destroy, :with => :id do
    ...
    if account
      if account != current_account && account.destroy
        flash[:success] = pat(:delete_success, :model => 'Cuenta', :id => "#{params[:id]}")
      else
  ...
  delete :destroy_many do
    ...
    if accounts.include? current_account
      ...
      flash[:success] = pat(:destroy_many_success, :model => 'Cuenta', :ids => "#{ids.join(', ')}")
    end
    redirect url(:accounts, :index)
  end
end
